...
sim_coxred<- function(N,lambda0, beta, censor.right)
{
  # N = Total sample size 
  ...
  #young =1: if AGE<=55  =0 if age after 55
  young<- c(1:N)*0;
  for (i in 1:N) {
    if(initial$Age[i]<=59)
    {
  ...
  initial<-cbind(initial,young, sacrum, surgery, rad, RS, classic, chon)
  
  # generate underlying event time
  # T <- rweibull(n=N, shape=1, scale = lambda0*exp(beta[1]*young+beta[2]*surgery+beta[3]*sacrum))
  # 
  
  T <- rweibull(n=N, shape=1, scale = lambda0*exp(beta[1]*young+beta[2]*sacrum 
                                                  +beta[3]*surgery))
...
mydatared2<-sim_coxred(N=765, lambda0=55, beta=c(-log(1.052), -log(0.401),-log(0.291)), censor.right=500)
write.csv(mydatared2,
          file="/Users/sabrina/Desktop/Fall 2020/BST222/Project 2/mydatared3.csv", row.names = FALSE)

for (i  in 1:765) {
if(mydatared2$Diagtime[i]==2&mydatared2$Therapy[i]==1 & mydatared2$time[i]>10)
{
  mydatared2$time[i]<-mydatared2$time[i]+30
}
}

for (i  in 1:765) {
  if(mydatared2$Diagtime[i]==2)
    if((mydatared2$time[i]<=60))
  {
    mydatared2$time[i]<-min(mydatared2$time[i]-30, 1)
  }
}
write.csv(mydatared2,
          file="/Users/sabrina/Desktop/Fall 2020/BST222/Project 2/mydatared3.csv", row.names = FALSE)
...
sim_cox<- function(N,lambda0, beta, censor.right)
{
  ...
  diagtime<-sample(x=c( 1, 2, 3, 4), size=N, replace=TRUE, prob=c(0.089, 0.134, 0.262, 0.515))
  # #therapy =1: radiology =2:Surgery alone =3: Surgery and radiotherapy alone =4: neither surgery nor radiotherapy
  therapy<-sample(x=c( 1, 2, 3, 4), size=N, replace=TRUE, prob=c(0.055, 0.058, 0.821, 0.066))
  ...
  T <- rweibull(n=N, shape=1, scale = lambda0*exp(beta[1]*young+beta[2]*tumorSize+
                                                    beta[3]*marital + beta[4]*gender+
                                                    beta[5]*race+beta[6]*sacrum 
                                                  +beta[7]*surgery + beta[8]*rad +beta[9]*RS 
                                                  + beta[10]*classic + beta[11]*chon))
...
#lambda0=log(2)/median:52
mydata2<-sim_cox(N=765, lambda0=55, beta=c(-log(1.052), -log(1.001),-log(0.79), -log(1.138), -log(1.627), 
                                           -log(0.668), -log(0.288), -log(0.861), -log(0.524), -log(0.215),
                                           -log(0.507)), censor.right=500)
